...
def register_router_stats_hooks(model) -> None:
    for name, module in model.named_modules():
        ...
        def _hook(_module, _inputs, output, *, layer_name=name):
            ...
            if hasattr(output, 'indices'):
            ...
            elif hasattr(output, 'router_indices'):
                indices = getattr(output, 'router_indices')
            elif hasattr(output, 'expert_indices'):
                indices = getattr(output, 'expert_indices')
                ...
                if len(output) >= 2 and torch.is_tensor(output[1]) and output[1].dtype in (torch.int32, torch.int64):
                    indices = output[1]
                else:
                    for item in output:
...
def save_router_stats(iteration: int) -> None:
    ...
    counts = None
    if args.num_experts:
        counts = torch.bincount(torch.as_tensor(indices, dtype=torch.int64), minlength=args.num_experts).cpu().numpy()
    ...
    position_ids = _ROUTER_STATS.get('position_ids')
    if position_ids is not None:
        payload['position_ids'] = position_ids.cpu().numpy()
    sample_ids = _ROUTER_STATS.get('sample_ids')
    if sample_ids is not None:
        payload['sample_ids'] = sample_ids.cpu().numpy()
    import numpy as np
    np.savez(file_path, **payload)


def record_router_stats_inputs(batch: Dict[str, torch.Tensor]) -> None:
    if not batch:
        return
    for key in ('input_ids', 'position_ids', 'sample_ids'):
        if key in batch and batch[key] is not None and torch.is_tensor(batch[key]):
            _ROUTER_STATS[key] = batch[key].detach().cpu()
